...
export default function Home() {
  ...
  const brandBg = "bg-[radial-gradient(120%_100%_at_50%_-10%,rgba(106,0,255,.05),rgba(255,46,168,.04),transparent)]";
  ...
  return (
    <div className="min-h-screen flex flex-col text-slate-800 overflow-x-hidden">
      ...
      <main className={brandBg}>
        ...
        {/* Reviews — add anchor so header link /#reviews goes here */}
        <Section id="reviews">
          <ReviewsStrip />
        </Section>
      </main>
...
function Section({ children, muted = false, id, last = false }) {
  return (
    <section id={id} className={`${muted ? "bg-white/60" : "bg-transparent"}`}>
      ...
      {!last && <div className="h-px w-full bg-gradient-to-r from-transparent via-violet-200/60 to-transparent" />}
    </section>
  );
}
